...
function parseQuizQuestions(html: string): WeekQuiz[] {
  ...
  for (let i = 0; i < weekMatches.length; i++) {
    ...
    // Method 1: Parse questions 1-10 from the main ol structure
    // Look for the pattern: <li>Question text<ol><li>option1</li><li>option2</li>...</ol></li>
    const mainOlMatch = weekContent.match(/<ol>(.*?)<\/ol>/s)
    if (mainOlMatch) {
      const mainOlContent = mainOlMatch[1]
      
      // Split by individual question patterns - look for <li> followed by question text and nested ol
      ...
      for (const match of questionMatches) {
        ...
        if (options.length >= 2) {
          ...
          console.log(`  Found question (main): ${questionText.substring(0, 50)}... (${options.length} options, correct: ${correctAnswer})`)
        }
      }
    }
    
    // Method 2: Parse questions 11-16 (separate paragraphs with strong tags)
    // Look for pattern: <p><strong>Q#. Question</strong></p><ol><li>option1</li><li>option2</li>...</ol>
    ...
    if (questions.length < 16) {
      ...
      // Look for any remaining ol structures that might contain questions
